...
### 開発優先順位（2026-08-16追加・詳細はセクション11）
現在のAEO実装（構造化データ・7軸評価等）は「設計思想」と「実装済みの事実」を
分けて評価する。記事量産を最優先にせず、**構造化 → 検証 → 拡張**の順で
「記事サイト → 評価データベース → AI推薦基盤 → API・外部提供」へ発展させる。
詳細な優先順位・Phase定義・施設選定基準はセクション11
「AEO・AI推薦基盤 開発ロードマップ」を正とする。
...
## 11. AEO・AI推薦基盤 開発ロードマップ（2026-08-16策定）

> 本セクションは、TPJを単純な記事量産から
> **記事サイト → 構造化データ → 評価データベース → AI推薦基盤**
> へ発展させるための開発方針・優先順位・判断基準を定める。
> セクション5「事業背景」のAI戦略・ゴールを運用レベルまで具体化したものであり、
> `AEO-PLAYBOOK.md`（AEO実装アクション）・`AGENT-INSTRUCTIONS.md`（記事生成運用）・
> 本書セクション10（TPJセレクト掲載基準＝**新規掲載**の判断基準）とは
> 対象が異なる別レイヤーの方針であり、矛盾しないよう整合させている。
> 矛盾を感じたら本書（CLAUDE.md）を最上位とする。

### 11-1. 設計思想と実装済み価値を分離する（最重要）

TPJの「7軸評価 × 地域 × カテゴリ」という設計思想自体は高く評価できる一方、
以下は**設計上の可能性**であって**実装済みの事実**そのものではない。
...
「設計が優れている」ことを「すでにAI推薦基盤として完成している」と誤認しない。
提案・判断の際は、必ず**実装済みの事実**と**設計上の可能性**を分けて記述する。

参考（本セクション策定時点で確認できる実装済みの事実の一例）：
- TPJセレクト・認証施設：`venues.json` 掲載数は本セクション策定時点で35件
  （本書セクション10の段階的拡大計画に基づき今後も増加する）
- 構造化データ：全施設ページに `LocalBusiness` + `Review`/`reviewRating` +
  7軸スコア（`additionalProperty`）+ `geo` を実装済み。`openingHours`・
  `sameAs`・`amenityFeature`・`smokingAllowed` はグレード別に条件出力済み
  （正本：本書末尾「AEO・構造化データ実装ルール」、ルート `CLAUDE.md`
  「JSON-LD実装ルール」。**本セクションはこれらを重複して再定義しない**）
- AI引用・推薦実績：現時点では自社観測のみで、第三者検証済みの確定データは
  存在しない（11-10・11-11参照）

**本番反映の確認（2026-08-16実施）**：上記の構造化データが「データ定義のみ」
ではなく**本番デプロイ済み**であることを、実際に本番URLをfetchして確認した。

| 確認施設 | grade_slug | 確認結果 |
|---|---|---|
| Green Beans Coffee（`/stories/select/green-beans-coffee-shibuya/`） | flagship | `geo`・`openingHours`・`sameAs`・`amenityFeature`・`smokingAllowed`・7軸`additionalProperty`（10件）・`aggregateRating`・Review（`reviewRating`＋7軸`additionalProperty`7件）すべて出力を確認 |
| ONIBUS COFFEE 中目黒（`/stories/select/onibus-coffee-nakameguro/`） | select | `geo`・7軸`additionalProperty`（10件）・`aggregateRating`は出力、`openingHours`・`sameAs`・`amenityFeature`はグレード制御により**意図通り非出力**（Silver以上限定ルールが正しく機能）であることを確認 |

両施設ともJSON-LDのパースに成功し、ドキュメント記載のグレード別出力ルールと
本番HTMLの実際の出力が一致していた。したがって本セクションの「実装済み」
表記は「本番デプロイ済み」を意味するものとして扱ってよい。

### 11-2. TPJの発展モデル（4フェーズ）
...
最終的には、**「記事を書く会社」ではなく「AIが参照・評価・推薦できる
地域・施設データ基盤」**を目指す。セクション5「ゴール（将来）：
インバウンド・多業種横断の経済圏構築 → 上場可能な事業規模へ」を
実装レベルで具体化したものであり、対立する目標ではない。

記事は目的ではなく、**データ資産を形成するための手段**と位置づける。

```
記事 → 一次情報 → 構造化データ → 評価データ → AI推薦
```

### 11-3. 開発優先順位：「量産 → 検証」ではなく「構造化 → 検証 → 拡張」

今後は「記事を増やす」ことを最優先にしない。以下の順番を基本とする。

1. 既存施設・記事情報の構造化（11-5参照）
2. 7軸評価のデータベース化（11-6参照）
3. 利用目的・地域・カテゴリ等の意味情報追加（11-7参照）
4. AI引用・AI推薦への効果検証（11-11参照）
5. 検証結果を踏まえた記事・施設・地域の拡張

基本原則（11-14に再掲）：**量より構造。記事数より一次情報。キーワードより意味。
検索順位よりAIからの引用・推薦。記事メディアから評価データベースへ。**

### 11-4. 新規記事の運用方針（`AGENT-INSTRUCTIONS.md` と連動）

記事量産を完全に停止するのではなく、**構造化・既存記事強化を優先しながら、
戦略的に必要な記事のみ追加する。** 記事生成の標準フロー・セルフ検証・
エスカレーション基準そのものは `AGENT-INSTRUCTIONS.md` を正とし、
本節はその「STEP 3 重複チェック」で使う判断基準を定義する。

**新規記事の許可基準**（いずれかに該当すれば追加可）
- 既存記事がゼロの「カテゴリ × 主要エリア」を埋める
- TPJの地域 × カテゴリ構造上の重要な空白を補完する
- 新しい検索意図・質問意図を獲得できる
- 一次情報を追加できる
- 7軸評価などTPJ独自データを追加できる
- 将来的なデータベース/API化につながる

**原則保留の基準**
同一市区町村内で類似カテゴリをさらに細分化する記事は、
当該エリアの構造化が完了するまで原則保留する。
例：禅体験／自然体験、神社／文化施設、カフェ／サードプレイス、公園／散策スポット。

目的：カニバリゼーション防止・重複コンテンツ防止・薄いコンテンツ化防止・
既存コンテンツのデータ資産化。

> この基準は、ルート `CLAUDE.md`「TPJ ピラー記事 制作ルール」が定める
> 都ピラー→自治体ピラー→密集エリアピラーの立体構造・重複回避ルールと矛盾しない。
> 「地域×分野の面を敷き詰める」既存方針の範囲内で、**同一エリア内の過度な
> 細分化にだけ、より慎重な優先順位判断を追加する**ものである。

### 11-5. 施設ページの構造化データ（実装状況の確認と優先順位づけ）

`LocalBusiness`・`Review`・`reviewRating`・7軸スコア（`additionalProperty`）・
`geo`・`OpeningHoursSpecification`・`sameAs`・`amenityFeature`・`smokingAllowed`
は、グレード別の条件出力を含めてすでに実装済み（正本：本書末尾
「AEO・構造化データ実装ルール」、ルート `CLAUDE.md`「JSON-LD実装ルール」）。

今後の課題は新しいスキーマ種別の追加ではなく、**どの施設から優先的に
情報の厚み・鮮度・一次情報を高めるか**という優先順位づけである（11-10参照）。

### 11-6. 7軸評価のデータ構造（現状確認）

`venues.json` は既に施設ごとに以下を保持している。

```
venue
├─ grade / grade_slug（認証ステータス）
├─ scores（7軸の数値：comfort/silence/special/story/revisit/record/inbound）
├─ axes_primary / axes_secondary（主軸・補助軸）
├─ area_primary（地域）
├─ category / category_slug（カテゴリ）
├─ certified_date（掲載・認証日）
├─ tags（利用シーン・受け入れ条件等のタグ）
└─ amenity_features（Wi-Fi・電源・禁煙。tagsとは別の設備専用フィールド）
```

将来的には、地域別比較・カテゴリ別比較・利用目的別推薦・AI検索・AI推薦・
API提供・外部企業へのデータ提供へ発展可能な構造として、この既存スキーマを
土台に**利用目的タグの体系化（11-7）**と**最終確認日の明示（11-8）**の
2点を追加整備する。この2点以外は新規のデータモデル設計を要しない。

### 11-7. 利用目的タグの体系化（3層の区別を明確化・2026-08-16修正）

初稿では「利用シーン系」と「受け入れ・対応系」を単純に「利用目的タグ」として
一体運用するとしていたが、レビューで**設備（アメニティ）と利用目的（シーン）を
混同している**指摘を受け、以下の3層に整理し直す。

| 層 | 意味 | 該当するタグマスターリストのグループ | 格納フィールド |
|---|---|---|---|
| ① 利用目的（シーン） | 「何のために来るか」 | 「利用シーン系」（読書・集中・作業・ワーケーション・テレワーク・朝活・夜カフェ・気分転換・デート・一人時間・一人カフェ・一人ランチ等） | `venue.tags` |
| ② 受け入れ・対応条件 | 「誰を／何を受け入れるか」 | 「受け入れ・対応系」（ペット同伴・グルテンフリー・ビジネスパーソン・商談等） | `venue.tags` |
| ③ 設備（アメニティ） | 「何が物理的に備わっているか」 | タグマスターリスト外。真偽値で管理する事実 | `venue.amenity_features`（Wi-Fi・電源・禁煙。**`tags`とは別フィールド。混在させない**） |

**運用ルール（新設）**
- ①②はいずれも`venue.tags`に格納される点で実装上は同じフィールドだが、
  クエリ意図としては別物である（①＝目的、②＝条件）。タグ選定時は
  どちらの層に属するタグかを意識して選ぶ
- ③（Wi-Fi・電源・禁煙等の設備事実）は`tags`に**入れない**。既存の
  `amenity_features`フィールド・「施設情報カード 設備バッジルール」
  （ルート`CLAUDE.md`）が正本であり、本節で新しい格納先は作らない
- タグ選定基準・グレード別上限数は既存ルール（タグマスターリスト）を
  継続適用し、①②③いずれについても別体系は作らない
- 施設追加・編集時は、その施設が答えられる利用目的クエリを意識してタグを選ぶ
  - 例：「東京で一人で仕事ができる場所」→ ①作業・一人時間（`tags`）＋
    ③Wi-Fi・電源の実際の有無（`amenity_features`）
  - 例：「北参道で読書に向いているカフェ」→ ①読書・静か・一人カフェ（`tags`）
  - 例：「外国人旅行者におすすめのサードプレイス」→ インバウンド適性軸＋
    ①②の該当タグ
- 新しい利用目的・受け入れ条件の需要が見つかった場合は、既存の
  「タグマスターリスト」に正式に追加してから使用する（本書運用ルールと同じ、
...
インバウンド・海外AI検索への対応として、英語ページ・英語情報の量と品質を
確認する。単純な機械翻訳による記事量産ではなく、以下を優先する。

- 施設情報（`body_en`・`faq_groups_en`・`tags_en`）
- 地域情報
- 利用目的
- 7軸評価
- TPJ認証
...
> 既存の英語版運用ルール（`body_en`/`faq_en`の充実義務、日本語版拡充時の
> 英語版同時更新、`hreflang`双方向設定等）は、ルート `CLAUDE.md`
> 「英語版サードプレイスガイド運用ルール」および「venues.json 任意フィールド
> 一覧」に既に詳細に定義済みであり、本節はこれを重複定義しない。
> 本節が新たに求めるのは、**Phase 1対象施設（11-10）における英語情報の
> 充足率チェックを、構造化データ整備の一項目として明示的に含めること**である。

### 11-10. Phase 1対象施設の選定（現行TPJセレクト・認証施設が対象）

本書セクション10「TPJセレクト掲載基準ルールブック」は**新規に掲載する
施設を選ぶ基準**である。これに対し、ここで定めるPhase 1選定は、
**既に掲載済みの施設のうち、構造化・情報強化を先行して行う施設を選ぶ基準**
であり、両者は別の判断である。混同しないこと。

> **実行結果は `PHASE1-TARGET-LIST.md` を正とする（2026-08-16作成）**。
> 全35件（`venues.json` 掲載施設）の一覧・優先度A/B/C仕分け・優先度A
> 8件のPhase 1第一弾対象リストは、基準の重複記載を避けるため本書ではなく
> 同ファイルに一元化している。以下は判定基準（何を根拠に仕分けるか）のみを
> 定義し、判定結果そのものは記載しない。
...
**判定者・判定プロセス（2026-08-16追加）**
優先度A〜Cの判定、特に定性的な「TPJブランドを代表する」「モデルケースとして
活用しやすい」の判断は、**TPJ編集部が合議で行い、最終判断は編集長（現在は
加瀬俊幸）が行う**（既存の「author/editor（E-E-A-T）と取材根拠の表示ルール」
（ルート`CLAUDE.md`）で編集長の実在を明示している役割を、社内の判定プロセス
にもそのまま適用する）。判断が割れた場合は、以下の準客観的な補助基準で
優先順位を確定する。

- グレード（Flagship＞Platinum＞Gold＞Silver＞Certified＞TPJセレクト）
- 実証店舗であるか（実証店舗＝Flagshipの部分集合。詳細はルート`CLAUDE.md`
  「『実証店舗』と『TPJ認証』の関係」）
- 7軸評価スコアの平均値・充実度
- `body`/`faq_groups`等の一次情報量（文字数・FAQ問数）

「TPJブランドを代表する」を無根拠な主観判断のままにせず、判定理由を
一言メモとして残すことを推奨する（後日の棚卸しで判断根拠を追跡できるようにするため）。

**選定基準に「AI引用実績」は使用しない（重要）**
現時点のAI引用・AI推薦実績は自社観測であり、第三者検証済みデータと
明確に区別する（11-1参照）。したがって、AI引用実績をPhase 1対象施設の
主要な選定基準には使用しない。施設選定には、認証グレード・TPJ上の重要性・
7軸評価データ・一次情報量・PV・検索流入・地域/カテゴリ上の重要性など、
現時点で確認可能なデータのみを使用する。AI引用・AI推薦は、
**「構造化データ実装後の効果検証項目」**として扱う（11-11参照）。

**実装手順**
1. 現在のTPJセレクト・認証施設一覧を確定する（`venues.json` を正とする）
2. 各施設について、認証グレード・7軸評価の有無・一次情報量・PV・検索流入・
   カテゴリ・エリア・利用目的タグ・最終確認日の有無を整理する
3. 優先順位（A/B/C）を付ける
4. Phase 1対象施設を確定する
5. 対象施設へ、Schema.org構造化データ（既実装分の点検含む）・7軸評価の
   数値化点検・利用目的タグ・地域・カテゴリ・認証情報・最終確認日を整備する
...
AI引用・AI推薦については、現時点では**自社観測**として扱い、
第三者検証済みの確定データとは区別する。月次の測定プロセスは
`AEO-PLAYBOOK.md`「6. 効果測定」の運用（月1回のAI検索実査・Search Console
確認）をそのまま用い、本節のために別の測定プロセスを新設しない。

**Phase 1優先度A・8施設の具体的な観測計画（比較基準日・観測項目・観測開始
タイミング）は `PHASE1-EFFECT-VERIFICATION-PLAN.md`（2026-08-17策定）を
正本とする。** 同ファイルは、構造化データの本番稼働開始日（施設ごとに
2026-07-06〜07-21と幅がある）と`last_verified_date`の本番反映日
（2026-08-17）が異なる点を踏まえ、基準日の設計・観測項目・記録テンプレート
まで具体化している。

### 11-12. コンテンツ追加の判断基準（記事・施設・カテゴリ・地域 共通）

記事・施設・カテゴリ・地域を追加する際は、単純な記事数・施設数では判断しない。
以下を判断基準とする。
...
- 一次情報を追加できるか
- 7軸評価による独自データを追加できるか
- AIが引用・比較・推薦するための新しい意味情報になるか
- 将来的なデータベース/API化につながるか

### 11-13. 実装開始条件

本ロードマップに基づく実装は、以下の順で開始する。

1. TPJセレクト・認証施設の現状一覧を確定
2. 各施設の認証・7軸評価・一次情報・PV・検索流入等を整理
3. Phase 1対象施設を確定
4. 構造化データの標準仕様を点検・確定（既存実装の棚卸し。11-5参照）
5. 7軸評価・利用目的等のデータ項目を定義（11-7・11-8参照）
6. 対象施設へ試験実装
7. 実装前後の検索・AI・流入データを比較（11-11参照）
8. 効果検証
9. 標準モデルを確定
10. 他施設・他地域へ展開

**記事量産の拡大は、この検証結果を踏まえて再設計する。**

### 11-14. TPJの基本原則（AEO・コンテンツ戦略）

**量より構造。記事数より一次情報。キーワードより意味。
検索順位よりAIからの引用・推薦。記事メディアから評価データベースへ。**
