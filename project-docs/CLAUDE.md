...
> 有償展開：法人向けサービス（掲載枠・認証グレードSilver以上・コンサル等）の問い合わせ窓口は
> メインサイト（`#services`・`#for-business`）に静かに設置済み。積極的な営業・勧誘・価格の
> 前面訴求は行わない。記事サイト（/stories/）から有償サービスへの直接誘導は原則禁止とする
> （認証制度の"仕組みの説明"として自然に必要な箇所のみ例外）。
>
...
- 目的：認証制度・思想への理解と共感。法人向けサービスの問い合わせ窓口
  （`#services`・`#for-business`）は静かに設置済みだが、積極的な営業・勧誘・
  価格の前面訴求は行わない
...
- トーン：宣伝色を排し、読み物として完成。店舗記事も「広告」でなく「編集」として載せる。
  記事内から有償サービスへの直接誘導は原則禁止（認証制度の"仕組みの説明"として
  自然に必要な箇所のみ例外）
...
TPJセレクト・認証グレード関連のページ・記事では、金銭や商取引を連想させる表現を避け、
「体験・空間・文化」の文脈で語ること。
...
TPJセレクトおよびTPJ認証グレード関連ページ（施設詳細・FAQ・実証店舗セクション等）の
本文テキストは原則 **こげ茶（`#3D2B1F`）** を使用する。
...
**理由：** 黒（`#1C1C1C`）は主張が強く、TPJの「静か・品がある」トーンに合わない。
こげ茶は温かみがあり、権威と親しみのバランスを保つ。

**注意：** `text-dark`（`#1C1C1C`）を施設ページの本文テキストに使うことは禁止。
見出し（`h1`/`h2`）・施設名など「強調が必要な箇所」のみ `text-dark` を許可する。
...
8. **申請への導線（設置済み）** — For Business（`#for-business`）に問い合わせフォームを
   控えめな形で設置。積極的な営業・勧誘は行わない

※ 現フェーズ方針：メインサイトのCTA（認証申請・お問い合わせ誘導等）および「For Business」
　ページは、控えめなトーンで設置済み。積極的な営業・勧誘・価格の前面訴求は行わない。
　集客の主戦場は引き続き記事サイト（/stories/）の無償記事とし、
　記事サイトから有償サービスへの直接誘導は原則禁止とする
　（認証制度の"仕組みの説明"として自然に必要な箇所のみ例外）。
...
> **記事サイトは引き続き無償記事の充実に注力し、記事内から有償サービスへの直接誘導は
> 原則禁止とする**（認証制度の"仕組みの説明"として自然に必要な箇所のみ例外）。

---

## 5. 事業背景（※収益・価格等の内部設計は出さない）

制作判断の背景として保持。数値・価格・事業性の詳細は表面化させないこと。

> ＜現フェーズ方針＞ 下記「収益展開」の各メニューは、問い合わせ窓口としては
> メインサイト（`#services`・`#for-business`）に静かに設置済み。ただし価格・料金・
> 具体的な取引条件はサイトに一切出さず、積極的な営業・勧誘も行わない。
> 上場・収益構造・市場規模などの事業性の数字は引き続きサイト表面に出さない。
> 詳細な有償設計は別途非公開で保管する有償設計資料に定義。

### 収益展開（※問い合わせ窓口は設置済み・価格等の詳細は非公開）
- **認証**：無償認証で裾野を広げる（上位グレード・更新の有償化は将来）
- **記事投稿（有償）**：店舗のブランディング・集客・インバウンド訴求記事。
  `#services`の「PR記事制作」として問い合わせ受付中（価格は非公開）
- **掲載枠（デジタル不動産）**：地域×業種の限定枠（3枠/1枠）。`#services`の
  「掲載枠」として問い合わせ受付中。月額掲載料・枠の販売譲渡等の条件は非公開
- **コンサル（有償）**：サードプレイス認証取得・空間プロデュース支援。
  `#services`の空間プロデュース等のメニューとして問い合わせ受付中（価格は非公開）
...
**制作上の鉄則：収益構造・上場・市場規模等の事業性の数字はコピーやUIに翻訳して出さない。
事業性は"基準の信頼性"と"文化的価値"の見せ方を通じて、間接的にのみ表現する。
有償メニューの問い合わせ窓口自体は静かに設置してよいが、価格・料金・取引条件は
サイトに出さない。具体的な価格設計は、非公開で保管する有償設計資料に従う。**
...
GBC（Green Beans Coffee）は、その価値観を実際の店舗で試し、検証し続けている場所である。
席の間隔、光の当て方、音の設計、声をかけるタイミング——ひとつひとつを7基準に照らし、積み重ねた知見を評価基準そのものに還元している。
理念は、そこに「在る」ことで初めて、伝わるものになる。

### 8. For Business ─ 丁寧に場をつくる方へ
メインサイトのCTA・申請導線・お問い合わせ誘導（`#for-business`）は、控えめなトーンで
設置済み。積極的な営業・勧誘・価格の前面訴求は行わない。以下は実際にメインサイトで
使用している確定コピー（`public/index.html` と同期させること）。

丁寧に場をつくる方へ
Certified以上の正式認証へのお申し込み、Silver以上の認証審査へのお問い合わせ、各種ご支援のご相談はすべてこちらから。
［ お問い合わせ ］　Stories（記事を読む）→
...
- [ ] メインサイトのCTA・問い合わせ導線（`#services`・`#for-business`）が、
      積極的な営業・勧誘・価格の前面訴求になっていないか（控えめなトーンか）
- [ ] （現フェーズ）記事サイト（/stories/）本文から有償サービスへ直接誘導する
      導線を新たに設けていないか（認証制度の"仕組みの説明"として自然に必要な
      箇所のみ例外）
...
- **TPPの問い合わせ窓口は、メインサイト`#services`の「7つの基準のプロダクト化」
  メニューとして静かに設置済み。ただし販売・課金導線（購入・決済フロー）は設けず、
  地域×プロダクトの限定枠・ライセンス・購買直結AEO等の具体的な有償展開の詳細は
  引き続き非公開・未実装とする**（積極的な営業・勧誘は行わない）。
  サイト表面では思想・概念・第1号事例の紹介に留める。
...
- [ ] （現フェーズ）販売・課金（購入・決済）導線を新たに設けていないか
...
**改訂履歴（2026-07-20）**：カフェの目標比率を40%→30%に引き下げ、余剰の10ポイントを宿泊（15%→20%）とコワーキング（10%→15%）に再配分した。ウェルネス・その他は据え置き。理由：カフェの実掲載数は据え置いたまま総掲載数を増やす方針のため、目標値も実態に合わせて引き下げた。
...
### TPJセレクトのURL管理ルール（2026-07-04確定）

**確定URL構成：**

| ページ | URL |
|---|---|
| TPJセレクト一覧（日本語） | `/stories/select/` |
| 施設詳細（日本語） | `/stories/select/[slug]/` |
| 施設詳細（英語） | `/en/stories/select/[slug]/` |

**テンプレートファイルの場所：**
- 一覧ページ：`src/stories/select/index.njk`
- 施設詳細（日本語）：`src/stories/select/venue.njk`
- 施設詳細（英語）：`src/stories/select/venue-en.njk`
- 施設詳細レイアウト：`src/_layouts/venue.njk` / `src/_layouts/venue-en.njk`

**リダイレクト設定（`public/_redirects`）：**
```
/stories/certified/:splat   /stories/select/:splat      301
/stories/certified          /stories/select/            301
/en/stories/certified/:splat /en/stories/select/:splat  301
/en/stories/certified       /en/stories/select/         301
```

**内部リンクのルール：**
- 記事・テンプレートで施設一覧へリンクする場合は必ず `/stories/select/` を使う
- `/stories/certified/` は旧URLのため、新規記述では絶対に使わない
- URLを変更する際は `src/` 内を `stories/select` で検索して網羅的に確認すること
  - 対象：`.njk`テンプレート・`.md`記事ファイル・JSON-LD内のURL

**サイトマップへの反映：**
- `src/sitemap.njk` が新URLを自動生成する（`npm run build` 後に `public/sitemap.xml` に反映）
- Google Search Consoleに変更を通知すること
...
#### TPJセレクトURLを /stories/certified/ → /stories/select/ に移行（301リダイレクト）

**対象ファイル**
- `src/stories/select/` — 新規作成（index.njk, venue.njk, venue-en.njk）
- `src/stories/certified/` — テンプレート3件削除（index.njk, venue.njk, venue-en.njk）
- `public/_redirects` — 301リダイレクト追加
- `src/_layouts/venue.njk` / `src/_layouts/venue-en.njk` — URL全更新
- `src/_includes/nav-stories.njk` / `footer-stories.njk` / `src/stories/index.njk` — リンク更新
- 日本語記事8本・英語記事3本・`src/sitemap.njk` — 内部リンク更新

**変更内容**
- 新URL：`/stories/select/` および `/stories/select/[slug]/`
- 旧URL（`/stories/certified/`）からの301リダイレクトを `public/_redirects` に設定
- 英語版は `/en/stories/select/[slug]/` に移行

**ルール化**：「TPJセレクトのURL管理ルール」としてセクション10に追記済み。
