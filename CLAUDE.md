...
禁止事項（コピー・UI・記事・JSON-LDを問わず）：
- 収益モデル・料金・売上・市場規模・数値目標
- 上場・IPO・投資家・ロードマップ・フェーズ
- デジタル不動産の価格設計・利回り・回収期間
- 「事業の本命」「ハイリスク・ハイリターン」などの事業評価表現
- API提供・データ卸・送客課金などの収益スキーム

事業性は「基準の信頼性」と「文化的価値」の見せ方を通じて、間接的にのみ表現する。
BUSINESS-MODEL.mdを参照するのは制作判断の背景として確認する場合のみ。
